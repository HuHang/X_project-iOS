    ...
    if (device) {
        ...
        if (status == AVAuthorizationStatusNotDetermined) {
            [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
                if (granted) {
                    [weakself.view addSubview:weakself.scanningView];
                    [weakself setupSGQRCodeScanning];
                } else {
            ...
            [PCMBProgressHUD showLoadingTipsInView:self.view title:@"抱歉" detail:@"系统原因, 无法访问相机" withIsAutoHide:YES];
        }
    } else {
